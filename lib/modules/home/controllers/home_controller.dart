...
class HomeController extends GetxController {
  ...
  List<ItemModel> get filteredItems {
    ...
    if (searchQuery.isNotEmpty) {
      result = result.where((item) => item.name.toLowerCase().contains(searchQuery.value.toLowerCase())).toList();
    }
  ...
  Future<void> printPdf({List<ItemModel>? itemsToPrint, String? title}) async {
        ...
        build: (pw.Context context) {
          return [
            ...
            pw.Table.fromTextArray(
              context: context,
              headers: ['تاريخ الانتهاء', 'النظام', 'العرض', 'المخازن', 'الصنف'],
